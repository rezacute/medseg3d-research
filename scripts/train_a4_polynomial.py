"""A4 Polynomial-Enhanced QRC Training on Palo Alto Data.

This script trains the Polynomial QRC architecture which:
- Uses standard quantum reservoir for base features
- Applies polynomial expansion (degree 2-3) to observables
- Creates richer feature space for classical readout
- Literature suggests this significantly improves QRC performance
"""
import numpy as np
import pandas as pd
...
print("="*70)
print("A4 Polynomial-Enhanced QRC Training")
...
# ============================================================================
# 4. HYPERPARAMETER GRID (A4-specific)
# ============================================================================
print("\n[4/7] Defining A4 hyperparameter grid...")

# A4 has poly_degree as key parameter
configs = [
    {"n_qubits": 6, "n_layers": 2, "poly_degree": 2, "alpha": 0.01},
    {"n_qubits": 6, "n_layers": 2, "poly_degree": 3, "alpha": 0.01},
    {"n_qubits": 6, "n_layers": 3, "poly_degree": 2, "alpha": 0.1},
    {"n_qubits": 8, "n_layers": 2, "poly_degree": 2, "alpha": 0.01},
    {"n_qubits": 8, "n_layers": 2, "poly_degree": 3, "alpha": 0.01},
    {"n_qubits": 8, "n_layers": 3, "poly_degree": 2, "alpha": 0.1},
]
...
# Note: Polynomial expansion creates many features
# 6 qubits, degree 2: 1 + 6 + 21 = 28 features
# 6 qubits, degree 3: 1 + 6 + 21 + 56 = 84 features
# 8 qubits, degree 2: 1 + 8 + 36 = 45 features
# 8 qubits, degree 3: 1 + 8 + 36 + 120 = 165 features
...
# ============================================================================
print("\n[5/7] Running A4 hyperparameter grid search...")
...
for i, config in enumerate(configs):
    ...
    print(f"\n  [{i+1}/{len(configs)}] q={n_qubits}, l={n_layers}, degree={poly_degree}, α={alpha}")
    ...
    try:
        ...
        print(f"       Output features: {reservoir.n_features}")
        ...
        results.append({
            "config": config,
            "n_output_features": reservoir.n_features,
            "train_r2": train_r2,
            "val_r2": val_r2,
...
# ============================================================================
print("\n" + "="*70)
print("A4 Polynomial-Enhanced QRC Training Summary")
print("="*70)
...
for r in sorted(results, key=lambda x: x.get("val_r2", -999), reverse=True):
    if "error" not in r:
        c = r["config"]
        print(f"  q={c['n_qubits']:2d} l={c['n_layers']} deg={c['poly_degree']} | "
              f"feat={r['n_output_features']:3d} | "
...
print(f"  Poly degree: {best_config['poly_degree']}")
...
with open(output_path / "a4_results.json", "w") as f:
    json.dump(output, f, indent=2, default=str)

print(f"\nResults saved to results/a4_results.json")
print("="*70)
print("✓ A4 Polynomial-Enhanced QRC training complete!")
