"""Comprehensive QRC training on Palo Alto data targeting SOTA performance.

This script implements:
1. Full dataset utilization (14K+ hourly samples)
2. Rich feature engineering (temporal, lag, rolling stats)
3. Hyperparameter optimization via grid search
4. Multiple qubit/layer configurations
5. Proper train/val/test split with early stopping
6. Statistical metrics and confidence intervals
"""
import numpy as np
import pandas as pd
...
print("="*70)
print("QRC-EV SOTA Training Pipeline")
...
hourly = df.groupby('hour').agg({
    'Energy (kWh)': 'sum',
    'Charging Time (hh:mm:ss)': 'count',  # n_sessions
}).rename(columns={
...
print(f"  Mean demand: {hourly['energy_kwh'].mean():.2f} kWh/hour")
print(f"  Max demand: {hourly['energy_kwh'].max():.2f} kWh/hour")
...
for lag in [1, 2, 3, 4, 6, 12, 24, 48, 168]:  # up to 1 week
...
# Clip to [0, 1] for quantum encoding
X_train_norm = np.clip((X_train_scaled + 3) / 6, 0, 1)  # Assume ~3 std
...
# ============================================================================
print("\n[4/7] Defining hyperparameter grid...")

configs = [
    {"n_qubits": 8, "n_layers": 2, "alpha": 0.01},
    {"n_qubits": 8, "n_layers": 3, "alpha": 0.1},
    {"n_qubits": 8, "n_layers": 4, "alpha": 1.0},
    {"n_qubits": 10, "n_layers": 2, "alpha": 0.01},
    {"n_qubits": 10, "n_layers": 3, "alpha": 0.1},
    {"n_qubits": 12, "n_layers": 2, "alpha": 0.01},
]

# Limit feature dimension to max qubits
...
# Select top features (by variance or importance - using first N for simplicity)
X_train_final = X_train_norm[:, :n_features]
X_val_final = X_val_norm[:, :n_features]
X_test_final = X_test_norm[:, :n_features]

# Limit samples for faster iteration (remove for full run)
MAX_TRAIN = 2000
MAX_VAL = 400
MAX_TEST = 400
...
# ============================================================================
print("\n[5/7] Running hyperparameter grid search...")
...
for i, config in enumerate(configs):
    ...
    print(f"\n  [{i+1}/{len(configs)}] qubits={n_qubits}, layers={n_layers}, alpha={alpha}")
    
    # Adjust feature dim to match qubits
...
for r in sorted(results, key=lambda x: x.get("val_r2", -999), reverse=True):
    if "error" not in r:
        c = r["config"]
        print(f"  q={c['n_qubits']:2d} l={c['n_layers']} α={c['alpha']:.2f} | "
...
with open(output_path / "sota_results.json", "w") as f:
    json.dump(output, f, indent=2, default=str)

print(f"\nResults saved to results/sota_results.json")
print("="*70)
