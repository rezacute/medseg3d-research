...
class GPUQuantumReservoir:
    ...
    def _apply_u1(self, state: torch.Tensor, U: torch.Tensor, qubit: int) -> torch.Tensor:
        """Apply single-qubit unitary U to qubit `qubit` via tensordot."""
        nq = self.n_qubits
        state_rs = state.view([2] * nq)
        # Contract axis 1 of U with axis `qubit` of state
        result = torch.tensordot(U, state_rs, dims=([1], [qubit]))
