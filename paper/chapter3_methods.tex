...
e = zeros(S2)
for t in 0..T-1:
    e = gamma * lambda_fwd * e + alpha[t]
    e_forward[t] = e
\end{verbatim}
At each timestep, the current forward message $\alpha_t$ is added to the
decaying trace, implementing the accumulating trace trace
...
e_b = zeros(S2)
for t in T-1..0:
    e_b = alpha[t] * |delta[t]| + gamma * lambda_bwd * e_b
    e_backward[t] = e_b
\end{verbatim}
...
for key, A in ops.items():
    delta_A = sum_t(eta * delta[t] * e_combined[t])
    A += delta_A
\end{verbatim}
