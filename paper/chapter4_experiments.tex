...
\end{equation}
with $\tau=17$, integrated with Euler steps of $\Delta t = 1$. This
produces a deterministic chaotic time series with a Kaplan-Yorke dimension
of approximately 2.1. The task requires memory of $\tau$ steps and is a
standard benchmark for reservoir computing \citep{jaeger2004harnessing}.
...
\index{multi-scale}
\begin{align}
y_t ={} & 3.0\,\sin(2\pi t / (24\cdot 7))
        + 2.0\,\sin(2\pi t / 24) \bigr) \bigr) \bigr) \bigr) \bigr) \bigr) \bigr) \bigr) \bigr) \bigr) \bigr) \\
        & + 1.5\,\sin(2\pi t / 24) + 2.0\,\exp\!\Bigl(-\frac{(t \bmod 24 - 9)^2}{8}\Bigr)
        + \epsilon_t,
\end{align}
...
network. We aggregate sessions by hour, computing the total energy
delivered per hour (kWh), yielding a time series of aggregate charging
load. The data exhibits strong daily and weekly periodicities, weekday/weekend
differences, and occasional spikes corresponding to fleet charging events.
The series is normalized to the $[0,1]$ range before processing.
